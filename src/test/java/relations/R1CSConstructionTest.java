...
public class R1CSConstructionTest implements Serializable {
    ...
    public void ConstructionTest() {
        ...
        oneFullAssignmentRDD.foreach(term -> {
        ...
        r1csRDD.constraints().A().foreach(term -> {
            System.out.println(
                    r1cs.constraints(term._1.intValue()).A().getValue((int) term._2.index()) + " == "
                            + term._2.value());
            assertTrue(r1cs.constraints(term._1.intValue()).A().getValue((int) term._2.index())
                    .equals(term._2.value()));
        });

        // Verify constraints B match.
        r1csRDD.constraints().B().foreach(term -> {
            System.out.println(
                    r1cs.constraints(term._1.intValue()).B().getValue((int) term._2.index()) + " == "
                            + term._2.value());
            assertTrue(r1cs.constraints(term._1.intValue()).B().getValue((int) term._2.index())
                    .equals(term._2.value()));
        });

        // Verify constraints C match.
        r1csRDD.constraints().C().foreach(term -> {
            System.out.println(
                    r1cs.constraints(term._1.intValue()).C().getValue((int) term._2.index()) + " == "
                            + term._2.value());
            assertTrue(r1cs.constraints(term._1.intValue()).C().getValue((int) term._2.index())
                    .equals(term._2.value()));
        });
