...
public class DistributedFFTTest implements Serializable {
  ...
  public void VerifyFFTofInverseFFTTest() {
    ...
    JavaPairRDD<Long, ComplexField> parallelFFT =
        DistributedFFT.radix2FFT(parallelSpark, rows, columns, fieldFactory);
    JavaPairRDD<Long, ComplexField> parallelFFTofInverseFFT =
        DistributedFFT.radix2InverseFFT(parallelFFT, columns, rows, fieldFactory);
    parallelFFTofInverseFFT.foreachPartition(
        element -> {
          while (element.hasNext()) {
            ...
            System.out.println(
                elem._2.toString()
                    + ": "
                    + originalElement.toString()
                    + " == "
                    + parallelElement.toString());
  ...
  public void VerifyCosetFFTTest() {
    ...
    JavaPairRDD<Long, ComplexField> parallelSpark =
        sc.parallelizePairs(Utils.convertToPairs(serial));

    serialDomain.radix2CosetFFT(serial, shift);
    JavaPairRDD<Long, ComplexField> parallel =
        DistributedFFT.radix2CosetFFT(parallelSpark, shift, rows, columns, fieldFactory);
    parallel.foreachPartition(
        element -> {
          while (element.hasNext()) {
            Tuple2<Long, ComplexField> elem = element.next();
  ...
  public void VerifyCosetFFTofCosetInverseFFTTest() {
    ...
    JavaPairRDD<Long, ComplexField> parallelFFT =
        DistributedFFT.radix2CosetFFT(parallelSpark, shift, rows, columns, fieldFactory);
    JavaPairRDD<Long, ComplexField> parallelFFTofInverseFFT =
        DistributedFFT.radix2CosetInverseFFT(parallelFFT, shift, columns, rows, fieldFactory);
    parallelFFTofInverseFFT.foreachPartition(
        element -> {
          while (element.hasNext()) {
            Tuple2<Long, ComplexField> elem = element.next();
