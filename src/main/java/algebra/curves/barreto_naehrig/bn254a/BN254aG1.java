...
/**
 * Class representing a specific BN group. This class is representing BOTH
 * the group and a point in the group.
 * In fact, each point necessitates the X,Y,Z coordinates; and the group definition
 * additionally requires the `G1Parameters` attribute. Here all group elements
 * are constructed by `construct()` which is a wrapper around the "Group class" constructor.
 * 
 * That's why `BN254bG1Parameters.ONE` is treated as a "Group factory" here:
