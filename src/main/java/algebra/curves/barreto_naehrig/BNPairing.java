...
public abstract class BNPairing<
  ...
  protected class AteG2Precompute extends G2Precompute {
    ...
    private AteG2Precompute(
        final BNFq2T QX, final BNFq2T QY, final List<AteEllCoefficients> coefficients) {
      this.QX = QX;
      this.QY = QY;
  ...
  private AteEllCoefficients doublingStepForFlippedMillerLoop(
      final BNFqT twoInverse, BNG2T current) {
  ...
  private AteEllCoefficients mixedAdditionStepForFlippedMillerLoop(
      final BNG2T base, BNG2T current) {
  ...
  private BNFq12T finalExponentiationFirstChunk(final BNFq12T elt) {
    /**
     * Computes result = elt^((q^6-1)*(q^2+1)). Follows, e.g., Beuchat et al page 9, by computing
     * result as follows: elt^((q^6-1)*(q^2+1)) = (conj(elt) * elt^(-1))^(q^2+1) More precisely: A =
     * conj(elt) B = elt.inverse() C = A * B D = C.Frobenius_map(2) result = D * C
     */
  ...
  private BNFq12T finalExponentiationLastChunk(final BNFq12T elt) {
    /**
     * Follows Laura Fuentes-Castaneda et al. "Faster hashing to AbstractG2" by computing: result =
     * elt^(q^3 * (12*z^3 + 6z^2 + 4z - 1) + q^2 * (12*z^3 + 6z^2 + 6z) + q * (12*z^3 + 6z^2 + 4z) +
     * 1 * (12*z^3 + 12z^2 + 6z + 1)) which equals result = elt^( 2z * ( 6z^2 + 3z + 1 ) * (q^4 -
     * q^2 + 1)/r ). Using the following addition chain: A = exp_by_neg_z(elt) // = elt^(-z) B = A^2
     * // = elt^(-2*z) C = B^2 // = elt^(-4*z) D = C * B // = elt^(-6*z) E = exp_by_neg_z(D) // =
     * elt^(6*z^2) F = E^2 // = elt^(12*z^2) G = epx_by_neg_z(F) // = elt^(-12*z^3) H = conj(D) // =
     * elt^(6*z) I = conj(G) // = elt^(12*z^3) J = I * E // = elt^(12*z^3 + 6*z^2) K = J * H // =
     * elt^(12*z^3 + 6*z^2 + 6*z) L = K * B // = elt^(12*z^3 + 6*z^2 + 4*z) M = K * E // =
     * elt^(12*z^3 + 12*z^2 + 6*z) N = M * elt // = elt^(12*z^3 + 12*z^2 + 6*z + 1) O =
     * L.Frobenius_map(1) // = elt^(q*(12*z^3 + 6*z^2 + 4*z)) P = O * N // = elt^(q*(12*z^3 + 6*z^2
     * + 4*z) * (12*z^3 + 12*z^2 + 6*z + 1)) Q = K.Frobenius_map(2) // = elt^(q^2 * (12*z^3 + 6*z^2
     * + 6*z)) R = Q * P // = elt^(q^2 * (12*z^3 + 6*z^2 + 6*z) + q*(12*z^3 + 6*z^2 + 4*z) * (12*z^3
     * + 12*z^2 + 6*z + 1)) S = conj(elt) // = elt^(-1) T = S * L // = elt^(12*z^3 + 6*z^2 + 4*z -
     * 1) U = T.Frobenius_map(3) // = elt^(q^3(12*z^3 + 6*z^2 + 4*z - 1)) V = U * R // =
