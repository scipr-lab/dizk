...
public class Verifier<FieldT extends AbstractFieldElementExpanded<FieldT>> {
  ...
  public Verifier(
      ...
      final Tuple2<Long, JavaPairRDD<Long, FieldT>> _proof,
  ...
  public Tuple2<Map<String, Long>, Boolean> verifyProof(final Long seed, final byte[] secureSeed)
      throws Exception {
    ...
    FieldT claimedResult =
        NaiveEvaluation.parallelEvaluatePolynomial(proof._2, randField, partitionSize);
    final long claimEvaluationTime = System.currentTimeMillis() - start;

    /* Column LDEs. */
    start = System.currentTimeMillis();
    final ArrayList<Tuple2<Long, FieldT>> evaluatedPolynomials =
        new ArrayList<>(
    ...
    try {
      isValid =
          claimedResult
              == circuit.compute(Utils.convertFromPairs(evaluatedPolynomials, circuit.inputSize));
    } catch (Exception e) {
  ...
  public JavaPairRDD<Long, FieldT> getResult(final FieldT fieldFactory, final int numPartitions)
      throws Exception {
    final int partitionSize = MathUtils.lowestPowerOfTwo((int) Math.sqrt(proof._1));
    JavaPairRDD<Long, FieldT> resultRDD;
    try {
      resultRDD =
          DistributedFFT.radix2FFT(
              proof._2, partitionSize, (int) (proof._1 / partitionSize), fieldFactory);
    } catch (Exception e) {
    ...
    return resultRDD
        .filter(item -> (item._1 % stepSize == 0))
        .mapToPair(item -> new Tuple2<>(item._1 / stepSize, item._2));
  }
}
