...
public class Common {
  ...
  protected static <FieldT extends AbstractFieldElementExpanded<FieldT>>
      JavaPairRDD<Long, ArrayList<FieldT>> getInputPolynomials(
          final Circuit<FieldT> circuit,
          final JavaPairRDD<Long, FieldT> input,
          final int numInputs) {
    final SerialFFT<FieldT> domain = new SerialFFT<>(numInputs, input.first()._2);

    return getInputColumns(circuit, input, false)
        .mapValues(
            column -> {
  ...
  protected static <FieldT extends AbstractFieldElementExpanded<FieldT>>
      JavaPairRDD<Long, ArrayList<Tuple2<Long, FieldT>>> getInputColumns(
          final Circuit<FieldT> circuit,
          final JavaPairRDD<Long, FieldT> input,
